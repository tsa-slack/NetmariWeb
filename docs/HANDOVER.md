...
```mermaid
flowchart LR
    A[ページコンポーネント] -->|useQuery/useMutation| B[カスタムHooks]
    B -->|Repository呼び出し| C[Repository Layer]
    C -->|Supabase Client| D[Supabase REST API]
    D -->|RLS適用| E[PostgreSQL]
...
sequenceDiagram
    ...
    participant T as handle_new_user() トリガー
    participant DB as users テーブル

    U->>R: 登録情報入力
    R->>SA: signUp(email, password, metadata)
    SA->>T: AFTER INSERT トリガー
    T->>DB: INSERT INTO users (metadata から展開)
    SA-->>R: JWT トークン返却
    R-->>U: ログイン完了
```
...
```mermaid
flowchart LR
    A["/rental<br>日付・場所選択"] --> B["/rental/vehicles<br>車両選択"]
    B --> C["/rental/equipment<br>機器選択"]
    C --> D["/rental/activities<br>アクティビティ選択"]
    D --> E["/rental/confirm<br>確認・決済"]
```
...
`src/App.tsx` で全ルートを定義。全ページは `lazy()` + `Suspense` で遅延読み込み。
...
|---|---|
| `/portal` | ポータルトップ |
| `/portal/stories` | 体験記一覧 |
| `/portal/stories/new` | 体験記投稿 |
| `/portal/stories/:id` | 体験記詳細 |
| `/portal/stories/:id/edit` | 体験記編集 |
| `/portal/events` | イベント一覧 |
| `/portal/events/new` | イベント作成 |
| `/portal/events/:id` | イベント詳細 |
| `/portal/events/:id/edit` | イベント編集 |
| `/portal/questions` | Q&A一覧 |
| `/portal/questions/new` | 質問投稿 |
| `/portal/questions/:id` | 質問詳細 |
| `/portal/questions/:id/edit` | 質問編集 |
| `/portal/announcements` | お知らせ一覧 |

### マイページ
| パス | ページ |
|---|---|
| `/my`, `/my-page`, `/mypage` | マイページ（タブ: プロフィール/予約/お気に入り/体験記/レビュー/ルート/設定） |
...
| `/admin` | 管理ダッシュボード |
| `/admin/users` | ユーザー管理 |
| `/admin/vehicles` | レンタル車両管理 |
| `/admin/vehicles/new` | レンタル車両登録 |
| `/admin/vehicles/:id/edit` | レンタル車両編集 |
| `/admin/sale-vehicles` | 販売車両管理 |
| `/admin/sale-vehicles/new` | 販売車両登録 |
| `/admin/sale-vehicles/edit/:id` | 販売車両編集 |
| `/admin/equipment` | 機器管理 |
| `/admin/equipment/new` | 機器登録 |
| `/admin/equipment/:id/edit` | 機器編集 |
| `/admin/partners` | 協力店管理 |
| `/admin/partners/new` | 協力店登録 |
| `/admin/partners/:id/edit` | 協力店編集 |
...
本番環境では以下の環境変数をデプロイ先（Vercel/Netlify）の設定画面で登録:
...
| # | 課題 | 優先度 | 詳細 |
|---|---|---|---|
| 1 | ルートガードの不在 | 高 | Admin/Staff専用ページにクライアントサイドのルートガードがない。URLを直接入力でアクセス可能（RLSでデータ保護あり） |
